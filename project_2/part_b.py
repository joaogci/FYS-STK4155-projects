...
theta_SGD = optimizer_SGD.optimize(iter_max=int(1e4), eta=0.01, random_state=seed, tol=0)

optimizer_SGD = StochasticGradientDescent(lin_reg, size_minibatches=5)
theta_SGD = optimizer_SGD.optimize(iter_max=int(1e4), eta=0.01, random_state=seed, tol=1e-8)

optimizer_newton = NewtonMethod(lin_reg)
theta_newton = optimizer_newton.optimize(iter_max=int(1e4), eta=0, random_state=seed, tol=0)
...
plot_prediction_3D(theta_ols, 5, show=False, name="ols")
plot_prediction_3D(theta_SGD, 5, show=False, name="SGD")
plot_prediction_3D(theta_newton, 5, show=False, name="newton")
