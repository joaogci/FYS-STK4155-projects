...
print('THIS ONE DOESN\'T WORK YET, TEST WITH LOWER VALS BEFORE RUNNING! :)')
exit()
...
nn.grid_train(X_train, z_train, X_test, z_test, 'sigmoid_grid_search_eta_lambda_franke', plot=True, initial_learning_rate=np.logspace(-5, -1, 5), regularization=np.logspace(-15, -11, 5), epochs=epochs)

# Train with grid search - relu
nn = Model(2, cost_function=cost_fn, random_state=seed)
nn.add_layer(HiddenLayer(n_nodes, activation_function=ReLU()))
nn.add_layer(OutputLayer(1, activation_function=Linear()))
nn.grid_train(X_train, z_train, X_test, z_test, 'relu_grid_search_eta_lambda_franke', plot=True, initial_learning_rate=np.logspace(-5, -1, 5), regularization=np.logspace(-15, -11, 5), epochs=epochs)
