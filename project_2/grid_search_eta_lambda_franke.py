...
epochs = 500
n_nodes = 30
learning_rates = np.logspace(-7, -3, 5)
reg_params = np.logspace(-9, -5, 5)
...
nn.grid_train(X_train, z_train, X_test, z_test, 'sigmoid_grid_search_eta_lambda_franke', plot=True, initial_learning_rate=learning_rates, regularization=reg_params, epochs=epochs)

# Train with grid search - relu
nn = Model(2, cost_function=cost_fn, random_state=seed)
nn.add_layer(HiddenLayer(n_nodes, activation_function=ReLU()))
nn.add_layer(OutputLayer(1, activation_function=Linear()))
nn.grid_train(X_train, z_train, X_test, z_test, 'relu_grid_search_eta_lambda_franke', plot=True, initial_learning_rate=learning_rates, regularization=reg_params, epochs=epochs)
